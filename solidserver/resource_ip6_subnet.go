...
func resourceip6subnetExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	...
	http_resp, body, err := s.Request("get", "rest/ip6_block6_subnet6_info", &parameters)
...
func resourceip6subnetCreate(d *schema.ResourceData, meta interface{}) error {
	...
	block_id, err := ip6subnetidbyname(site_id, d.Get("block").(string), false, meta)
	if err != nil {
		// Reporting a failure
		return err
	}

	subnet_addresses, err := ip6subnetfindbysize(site_id, block_id, d.Get("size").(int), meta)
	if err != nil {
		// Reporting a failure
		return err
	}
	...
	for i := 0; i < len(subnet_addresses); i++ {
		...
		parameters.Add("subnet6_addr", hexip6toip6(subnet_addresses[i]))
		...
		if goffset != 0 {
			big_start_addr, _ := new(big.Int).SetString(subnet_addresses[i], 16)
		...
		http_resp, body, err := s.Request("post", "rest/ip6_subnet6_add", &parameters)
		...
		if err == nil {
			...
			if (http_resp.StatusCode == 200 || http_resp.StatusCode == 201) && len(buf) > 0 {
				if oid, oid_exist := buf[0]["ret_oid"].(string); oid_exist {
					...
					d.SetId(oid)
					d.Set("prefix", hexip6toip6(subnet_addresses[i])+"/"+strconv.Itoa(d.Get("size").(int)))
					if goffset != 0 {
...
func resourceip6subnetUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	http_resp, body, err := s.Request("put", "rest/ip6_subnet6_add", &parameters)
...
func resourceip6subnetgatewayDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if d.Get("gateway") != nil {
		...
		http_resp, body, err := s.Request("delete", "rest/ip6_address6_delete", &parameters)
...
func resourceip6subnetDelete(d *schema.ResourceData, meta interface{}) error {
	...
	// Sending the deletion request
	http_resp, body, err := s.Request("delete", "rest/ip6_subnet6_delete", &parameters)
...
func resourceip6subnetRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err == nil {
		...
		if http_resp.StatusCode == 200 && len(buf) > 0 {
			...
			retrieved_class_parameters, _ := url.ParseQuery(buf[0]["subnet6_class_parameters"].(string))
