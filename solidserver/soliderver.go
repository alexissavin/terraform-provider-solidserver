...
func (s *SOLIDserver) GetVersion() (error) {
  ...
  log.Println("[DEBUG] AdditionalTrustCertsFile = %s", s.AdditionalTrustCertsFile)
  ...
  response, body, err = apiclient.Get(fmt.Sprintf("%s/%s?%s", s.BaseUrl, "member_list", parameters.Encode())).
